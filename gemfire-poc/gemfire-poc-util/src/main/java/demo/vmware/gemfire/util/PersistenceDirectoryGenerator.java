...
public class PersistenceDirectoryGenerator
{
	...
	private Logger log = LoggerFactory.getLogger(PersistenceDirectoryGenerator.class);
			...
			if (!dir.exists())
			{
				if (!dir.mkdirs())
				{
					throw new RuntimeException(
							"Unable to create directory for persistence:" + dir);
				}
			}
	...
	private boolean lock(File directory)
	{
		...
		try
		{
			...
			log.trace("Attempting to lock:" + lockfile);
			lock = fc.tryLock();
			log.trace("Locked:" + lock);
			if (lock != null)
			{
